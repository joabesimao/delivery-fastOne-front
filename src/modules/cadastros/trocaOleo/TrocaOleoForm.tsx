...
const initialValues: TrocaOleoFormValues = {
  vehicleId: "",
  deliverymanId: "",
  km: "",
  changeDate: todayIso(),
...
const validate = (values: TrocaOleoFormValues): FormErrors => {
  ...
  if (!values.km || Number(values.km) <= 0) errors.km = "Informe o km lançado.";
  if (!values.changeDate) errors.changeDate = "Informe a data da troca.";
  return errors;
};

const TrocaOleoForm: React.FC<{ onSuccess?: () => void }> = ({ onSuccess }) => {
  ...
  const handleSubmit = async (
    values: TrocaOleoFormValues,
    { resetForm }: { resetForm: () => void }
  ) => {
    try {
      await api.post("/oil-change-log", {
        vehicleId: Number(values.vehicleId),
        deliverymanId: Number(values.deliverymanId),
        km: Number(values.km),
        changeDate: new Date(values.changeDate).toISOString(),
      });
      setSnackbar({ open: true, message: "Troca de óleo lançada com sucesso!", severity: "success" });
      resetForm();
      onSuccess?.();
    } catch (err) {
      setSnackbar({
        open: true,
        message: extractApiErrorMessage(err, "Erro ao lançar a troca de óleo. Tente novamente."),
        severity: "error",
      });
    }
  };
          ...
          {({ values, errors, touched, handleChange, handleBlur, isSubmitting }) => (
            <Form noValidate>
              ...
              <Grid container spacing={2} sx={{ mb: 1 }}>
                ...
                <Grid size={{ xs: 12, sm: 6 }}>
                  <FieldLabel label="Data da troca *" />
                  <TextField
                    fullWidth size="small"
                    type="date"
                    name="changeDate" value={values.changeDate}
                    onChange={handleChange} onBlur={handleBlur}
                    error={Boolean(touched.changeDate && errors.changeDate)}
                    helperText={touched.changeDate && errors.changeDate}
                  />
                </Grid>
              </Grid>
              ...
              >
                ...
                >
                  {isSubmitting ? "Salvando..." : "Lançar troca"}
                </Button>
              </Box>
            </Form>
          )}
